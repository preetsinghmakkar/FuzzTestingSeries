...
contract EchidnaTemplate {
    ...
    function add_test_associative(int128 x, int128 y, int128 z) public pure {
        int128 x_y = ABDKMath64x64.add(x, y);
        int128 x_y_z = ABDKMath64x64.add(x_y, z);

        int128 y_z = ABDKMath64x64.add(y, z);
        int128 y_z_x = ABDKMath64x64.add(x, y_z);
    ...
    function add_sub_inverse_operations(int128 x, int128 y) public {
        ...
        int128 x_y__y = ABDKMath64x64.sub(x_y, y);
        emit Debug(x_y__y, y);
        assert(x_y__y == x);
    }
    ...
    function div_test_not_commutative(int128 x, int128 y) public {
        //Through emit we came to know that echidna passing same and negative values also.
        // emit Debug(x, y);

        // So to stop it i am using pre-conditioning
        // Pre-condition
        ...
        int128 x_y = ABDKMath64x64.div(x, y);
        int128 y_x = ABDKMath64x64.div(y, x);
